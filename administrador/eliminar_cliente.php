...
function borrarCliente($id_cliente, $conn) {
    ...
    $stmt_check = $conn->prepare("SELECT * FROM reserva WHERE ID_CLIENTE = ? AND ESTADORESERVA = 'Confirmada'");
    $stmt_check->bind_param("i", $id_cliente);
    $stmt_check->execute();
    $result_check = $stmt_check->get_result();
    
    if ($result_check->num_rows > 0) {
        // Si tiene una reserva activa, no se puede borrar el cliente
        header("Location: eliminar_cliente.php?error=1");
        exit();
    }
...
<body>
    <div class="container">
        ...
        if (isset($_GET['error']) && $_GET['error'] == 1) {
            echo "<div class='alert alert-danger' role='alert'>No puede eliminar cliente ya que tiene una reserva activa.</div>";
        }
        ?>
