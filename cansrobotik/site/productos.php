...
<html class="wide wow-animation" lang="en">
                ...
                <div class="left-side">
                 ...
                 <div class="contact-info">
                  <div class="unit unit-middle unit-horizontal unit-spacing-xs">
                    ...
                    <div class="unit__body"><a class="text-middle" href="tel:#">+ (593) 0939798261</a></div>
                  </div>
                </div>
                </div>
          ...
          <div class="range range-xs-center">
            <div class="cell-xs-12 cell-xl-11">
              ...
              <ul class="breadcrumbs-custom">
                <li><a href="index.html">Robotik World</a></li>
  ...
  <div class="shell">
    ...
    <p>
      <b>En nuestra tienda, ofrecemos una selección de robots y herramientas interactivas diseñadas para que los niños aprendan mientras juegan. ¡Haz que el aprendizaje sea una aventura divertida!</b>
    </p>
    <div class="range range-30" id="productos-container" data-lightgallery="group">
      <?php
      // Incluir la conexión a la base de datos
      include '../../basedatos/basedatos.php';

      // Consulta para obtener los kits y sus imágenes
      $sql = "
        SELECT k.ID_KIT, k.NOMBRE, k.DESCRIPCION, k.PRECIO, k.STOCK, i.URL
        FROM kits k
        LEFT JOIN imagenes_kits i ON k.ID_KIT = i.ID_KIT
        ORDER BY k.ID_KIT;
      ";
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          $urlImagen = htmlspecialchars($row['URL']);
          $subtitulo = htmlspecialchars($row['PRECIO']);
          $id_kit = htmlspecialchars($row['ID_KIT']);
          $cantidad = htmlspecialchars($row['STOCK']);

          // Generar el HTML del producto
          $productosHTML .= "
            <div class=\"cell-sm-6 cell-md-4\">
              <a class=\"thumbnail-classic\" href=\"{$urlImagen}\" data-lightgallery=\"item\">
                <figure>
                  <img src=\"{$urlImagen}\" alt=\"{$nombre}\" width=\"370\" height=\"276\"/>
                </figure>
                <div>
                  <h4>{$nombre}</h4>
                  <p>{$descripcion}</p>
                ...
                <div class=\"caption\">
                  <p class=\"caption-title\">{$nombre}</p>
                  <p class=\"caption-text\">{$subtitulo}</p>
                </div>
              </a>
              <button class=\"btn-comprar\" onclick=\"redirectToForm('{$nombre}', '{$descripcion}', '{$subtitulo}', '{$id_kit}', '{$cantidad}')\">Comprar</button>
            </div>
          ";
...
<script>
  function redirectToForm(nombre, descripcion, precio, id_kit, cantidad) {
  // Codificar los parámetros para que puedan usarse en la URL
  const nombreEncoded = encodeURIComponent(nombre);
  const descripcionEncoded = encodeURIComponent(descripcion);
  const precioEncoded = encodeURIComponent(precio);

  // Obtener el ID del kit
  const ID_kit = encodeURIComponent(id_kit);

  // Obtener la cantidad de stock
  const Cantidad = encodeURIComponent(cantidad);

  // Redirigir a la página de confirmación con los parámetros en la URL
  window.location.href = `pedido/confirmar_reserva.php?nombre=${nombreEncoded}&descripcion=${descripcionEncoded}&precio=${precioEncoded}&id_kit=${ID_kit}&cantidad=${Cantidad}`;
}
