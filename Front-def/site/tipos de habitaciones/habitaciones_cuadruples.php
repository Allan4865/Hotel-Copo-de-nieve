<?php
    ...
    $resultado = $mysqli->query("SELECT * FROM habitaciones WHERE tipo='cuadruples'");
    ...
    $habitaciones = $resultado->fetch_all(MYSQLI_ASSOC);
     // Obtener las imágenes de las habitaciones de tipo 'dobles'
     $imagenes = [];
     $habitacionesIds = array_column($habitaciones, 'ID_HABITACION');
     if (!empty($habitacionesIds)) {
         $ids = implode(',', $habitacionesIds);
         $resultadoImagenes = $mysqli->query("SELECT url, id_habitacion FROM imagenes_habitaciones WHERE id_habitacion IN ($ids)");
 
         if (!$resultadoImagenes) {
             echo "<script>alert('Error al ejecutar la consulta de imágenes: " . $mysqli->error . "');</script>";
             exit();
         }
 
         // Convertir el resultado de imágenes en un array asociativo
         $imagenes = $resultadoImagenes->fetch_all(MYSQLI_ASSOC);
     }
?>
        ...
        <?php foreach ($habitaciones as $habitacion) : ?>
            <div class="col-md-4 mb-4">
                <div class="card h-100 shadow-sm">
                    <?php 
                        // Filtrar las imágenes correspondientes a la habitación actual
                        $imagenesHabitacion = array_filter($imagenes, function($imagen) use ($habitacion) {
                            return $imagen['id_habitacion'] == $habitacion['ID_HABITACION'];
                        });
                        
                        // Obtener la URL de la primera imagen (si existe)
                        $imagenHabitacion = reset($imagenesHabitacion);
                        $imagenUrl = $imagenHabitacion ? $imagenHabitacion['url'] : "../imagenes/default.jpg";
                    ?>
                    <img src="<?php echo $imagenUrl; ?>" alt="Imagen de la habitación" class="card-img-top img-fluid imagen-habitacion" style="object-fit: cover; height: 200px;">
                    <div class="card-body">
                    ...
                    <div class="card-footer">
                        ...
                        <form action="../habitaciones/detalle_habitacion_cuadruple.php" method="get">
...
<style>
    ...
    /*links botones redes*/
      ...
      document.querySelector('.card3').addEventListener('click', function() {
          var phoneNumber = '0959610758';
          var message = 'Hola, quisiera tener información más detallada sobre el hotel';
      ...
      document.querySelector('.card4').addEventListener('click', function() {
          var email = 'hotelcopodenieve@gmail.com'; // Reemplaza 'destinatario@example.com' con el correo electrónico del destinatario
          var subject = 'Información del Hotel';
          var body = 'Hola, quisiera tener información mas detallada sobre el hotel';

          var gmailComposeURL = 'https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=' + encodeURIComponent(email) + '&su=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(body);
